...
def _apply_location_hint(messages: list[dict], user_location: str | None) -> list[dict]:
    ...
    hint = (
        "\n\n## 사용자 지금 위치\n"
        + user_location
        + "\n\n[중요 - 위치 질문 대응] 사용자가 자기 위치·거주지·현재 있는 곳을 물어보면 "
        "(예: \"나 어디 사는지 알아?\", \"내 위치 알아?\", \"내가 어디 있게?\") "
        "반드시 위에 주어진 지역을 그대로 답해라. 모른 척 X, 회피 X. "
        "예: \"응, " + user_location + " 이잖아. 맞지?\"\n"
        + "\n[중요 - 통화 초반 지역 대화 · 2단계]\n"
        "**1단계** — 이 통화의 첫 번째 또는 두 번째 응답에서 반드시 사용자의 지역"
        "(" + user_location + ")을 언급하고 거기서 뭐 하는지 물어봐라.\n"
        "예:\n"
        "- \"지금 " + user_location + "네! " + user_location + "에서 뭐 하고 있어?\"\n"
        "- \"오, " + user_location + "야? " + user_location + "에서 뭐 하는 중이야?\"\n"
        "\n**2단계** — 사용자가 답하면 (예: \"집에서 쉬어\", \"친구랑 놀아\", \"일하는 중\") "
        "그 다음 응답에서 그 지역이 사용자에게 무엇인지 자연스럽게 추측·확인해라.\n"
        "예:\n"
        "- 사용자 \"집에서 쉬어\" → 너 \"아, 집이 " + user_location + "구나?\"\n"
        "- 사용자 \"친구랑 놀아\" → 너 \"" + user_location + "에 놀 데 많구나? 어디서 만나?\"\n"
        "- 사용자 \"일해\" → 너 \"" + user_location + "에서 일하는 거야? 회사가 여기야?\"\n"
        "목적: 지역·활동 매핑 학습 (다음 통화에서 활용).\n"
        "\n**지역 대화는 여기까지**. 2단계 확인 후 다시는 지역 얘기 반복 X. "
        "(사용자가 명시적으로 위치 물어보면 위 규칙 따라 답변)"
    )
