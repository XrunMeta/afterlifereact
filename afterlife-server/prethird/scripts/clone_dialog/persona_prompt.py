...
_RULE_ASKBACK = (
    ...
    # 아래 "핵심어 이어가기" 는 가비아 라이브에만 있던 것을 역흡수했다(2026-08-13).
    # 되묻기가 이미 답한 것을 원점에서 다시 묻는 실사용 증상에 대한 처방이다.
...
def bundle_to_messages(bundle: dict | None, speaker: dict | None = None) -> list[dict]:
    ...
    speaker = speaker or {}
    # [T-494 kill-switch · 라이브에서 역흡수 2026-08-13]
    #   =1 이면 미확정 상태를 무시하고 옛 계약(계정주 취급, L2 그대로 삽입)으로 돌아간다.
    #
    #   왜 필요했나: 이 파일의 Remember Me 변경이 **앱보다 먼저** 라이브에 배포되면서,
    #   서버는 신원 확인 전까지 대화를 막는데 앱에는 입력할 시트도 버튼도 없는 상태가 됐다.
    #   사용자가 신원을 넣을 방법이 없어 통화가 그대로 멈췄고(empty response), 다른 세션이
    #   이 스위치로 급히 막았다. 가비아에 실제로 =1 이 세팅돼 있었다.
    #
    #   즉 이것은 이 기능의 결함이라기보다 **반쪽 배포의 증상**이다. 세 계층(서버·API·앱)을
    #   함께 올린 뒤 =0 으로 내려 검증한다. 스위치는 남긴다 — 실통화에서 문제가 나면
    #   env 한 줄로 즉시 되돌릴 수 있는 편이 안전하다.
