...
def _apply_location_hint(
    ...
    intro_block = (
        "\n"
        "규칙 A. **통화 첫 응답 딱 1회만** 다음 형식으로 인사:\n"
        "  \"안녕 [상대이름]야/아 지금 " + user_location + "에 있네 뭐하고 있어?\"\n"
        "  (상대 이름은 위 '## 상대 정보' 의 '너가 상대를 부르는 호칭' 또는 speaker 이름 사용)\n"
        "  실제 예: \"안녕 지호야 지금 " + user_location + "에 있네 뭐하고 있어?\"\n"
        "\n"
        "규칙 B. 사용자가 답하면 (예: \"집에서 쉬어\" / \"일해\" / \"친구랑 놀아\") "
        "**다음 응답 딱 1회만** 지역 의미 확인:\n"
        "  - 사용자 \"집에서 쉬어\" → 너 \"아, 집이 " + user_location + "구나?\"\n"
        "  - 사용자 \"일해\" → 너 \"" + user_location + "에서 일하는 거야?\"\n"
        "  - 사용자 \"친구랑 놀아\" → 너 \"" + user_location + "에 놀 데 많구나? 어디서 만나?\"\n"
        "\n"
        "규칙 C. **매우 중요** — 규칙 A/B 각각 1회 사용 후엔 절대 반복 X.\n"
        "  이미 지역 인사·확인 했으면 이후 응답에서 **" + user_location + " 단어 자체를 언급 금지**.\n"
        "  대화 내역(history) 에 이미 \"" + user_location + "\" 가 있으면 규칙 A/B 는 이미 사용된 것.\n"
        "  → 새 화제로 넘어가라 (일반 되묻기, 사용자 발화 이어가기).\n"
        "  → 지역 반복 언급 시 통화 실패로 간주.\n"
        "\n"
