...
def _apply_location_hint(
    ...
    intro_block = (
        "\n"
        "규칙 A. **assistant 이전 응답이 하나도 없을 때만 (즉 이 통화의 정말 첫 응답)** 다음 형식으로 인사:\n"
        "  \"안녕 [상대이름]야/아 지금 " + user_location + "에 있네 뭐하고 있어?\"\n"
        "  (상대 이름은 위 '## 상대 정보' 의 '너가 상대를 부르는 호칭' 또는 speaker 이름 사용)\n"
        "  이미 assistant 가 한 마디라도 했다면 이 규칙 무시.\n"
        "\n"
        "규칙 B. 규칙 A 사용 직후 (assistant 이전 응답 1개, 그게 지역 인사) 사용자가 답한 경우만, "
        "**딱 1회** 지역 의미 확인:\n"
        "  - 사용자 \"집에서 쉬어\" → 너 \"아, 집이 " + user_location + "구나?\"\n"
        "  - 사용자 \"일해\" → 너 \"" + user_location + "에서 일하는 거야?\"\n"
        "\n"
        "규칙 C. **강력 금지** — assistant 이전 응답에 \"" + user_location + "\" 단어가 있거나, "
        "\"뭐하고 있어?\", \"뭐 해?\" 형태의 지역 인사 문구가 있으면 규칙 A/B 는 이미 사용된 상태.\n"
        "  → 지역 얘기 반복 절대 금지. \"" + user_location + "\" 단어를 다시는 언급하지 마라.\n"
        "  → 대신 사용자의 마지막 발화 내용을 이어받아 일반 대화 진행 (예: 사용자 '집에서 일해' → "
        "'어떤 일 해?' 또는 '피곤하지?' 등, 지역 언급 없이).\n"
        "\n"
    ...
    hint = (
        ...
        + intro_block +
        "규칙 D. 사용자가 자기 위치·거주지·집이 어딘지 물어보면 (\"나 어디 살아?\", "
        "\"내 집이 어디야?\", \"내 위치 알아?\", \"내가 어디 있게?\") **반드시 답변만** 하고 "
        "규칙 A(초반 인사) 형식으로 되돌아가지 마라.\n"
        "  형식: \"응, " + user_location + "이잖아. 맞지?\" (반말 유지, 존댓말 X)\n"
        "  실제 예:\n"
        "  - 사용자 \"내 집이 어디야?\" → 너 \"응, 니 집은 " + user_location + "이잖아.\"\n"
        "  - 사용자 \"나 어디 살아?\" → 너 \"" + user_location + "에 살잖아.\"\n"
        "  ❌ 절대 금지: \"" + user_location + "이시네요 뭐하시나요\" (규칙 A 형식 + 존댓말) → 사용자 질문 무시하고 인사로 되돌아감.\n"
        "  → 모른 척·회피·규칙 A 형식으로 되돌아가기 절대 금지."
    )
