...
def test_상태1_역할어_없음():
    ...
    [fix round 2 주석] 이 단언은 구조상 공허 통과다 — 아래 문자열은 입력에도
    템플릿에도 없어 어떤 구현이든 생성하지 않는다. 그래도 **의도한 템플릿 가드**라
    남긴다: 누가 머리말에 역할어를 넣으면 그 순간 깨진다(설계 3.1 근거).
    데이터 누출 검증이 아니라 문안 회귀 방지가 목적이라는 점을 명시해 둔다."""
    msgs = bundle_to_messages(_bundle_v({"displayName": "코조", "tone": "무뚝뚝함"}, "지호"))
    content = msgs[0]["content"]
...
def test_상대정보_블록이_없으면_그_블록을_가리키는_줄도_없다():
    ...
    [fix round 2] persona 에 상대 필드를 반드시 넣는다 — displayName·tone 뿐이면
    B-1 수정을 되돌려도(계정주 L2 폴백) 상대 줄이 0개라 블록이 안 생겨 이 단언이
    공허 통과한다."""
    for speaker in (
        ...
        content = bundle_to_messages(
            _bundle_v(dict(_L2_PERSONA), "지호"), speaker=speaker
        )[0]["content"]
        assert "## 상대 정보" not in content
        assert "지호와 어제 등산을 갔다" not in content
