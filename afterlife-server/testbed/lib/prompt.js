...
function replyLengthRule(longTail = '길게 설교하지 말 것.') {
  if ((process.env.REPLY_SHORT_MODE ?? '0') === '1') {
    const n = Number.parseInt(process.env.REPLY_MAX_CHARS ?? '30', 10) || 30;
    return `답은 반드시 한 문장으로만, 약 ${n}자 이내로 아주 짧게 하세요. 두 문장 이상 금지, 절대 길게 말하지 마세요.`;
  }
  return `답은 짧고 정겹게. 보통 1~3문장. ${longTail}`;
}
...
3. ${replyLengthRule(`${userRelation}이/가 묻는 거니까 길게 설교하지 말 것.`)}
