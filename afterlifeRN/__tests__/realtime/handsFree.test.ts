...
it('CALL_LIVE(micOff): paused, STT 시작 안 함', () => {
  const s = { phase: 'idle' as const, micOn: false, pendingText: '', confirmGate: false, signalGating: false };
...
it('CALL_LIVE(speaking 중 재발화): no-op — STT 강제 재시작 안 함', () => {
  const s = { phase: 'speaking' as const, micOn: true, pendingText: '', confirmGate: false, signalGating: false };
  const r = handsFreeReducer(s, { type: 'CALL_LIVE' });
...
it('FINAL_RESULT(게이트 OFF·listening, 텍스트): 즉시 sending + STOP_STT/SAY/START_DETECTOR + sayText, pendingText 유지', () => {
  const s = { phase: 'listening' as const, micOn: true, pendingText: '', confirmGate: false, signalGating: false };
...
it('즉발 정리: sending(게이트 OFF, pendingText 있음) + RESPONSE_END → listening, pendingText 비움 + STOP_DETECTOR/START_STT', () => {
  const s = { phase: 'sending' as const, micOn: true, pendingText: '안녕', confirmGate: false, signalGating: false };
...
it('즉발 정리: speaking(게이트 OFF, pendingText 있음) + RESPONSE_END → listening, pendingText 비움', () => {
  const s = { phase: 'speaking' as const, micOn: true, pendingText: '안녕', confirmGate: false, signalGating: false };
  const r = handsFreeReducer(s, { type: 'RESPONSE_END' });
  expect(r.state.phase).toBe('listening');
  expect(r.state.pendingText).toBe('');
});
...
it('FINAL_RESULT(게이트 ON·listening, 텍스트): confirming + pendingText, SAY 없음', () => {
  const s = { phase: 'listening' as const, micOn: true, pendingText: '', confirmGate: true, signalGating: false };
  const r = handsFreeReducer(s, { type: 'FINAL_RESULT', text: '  안녕  ' });
...
it('FINAL_RESULT(게이트 ON·confirming, 추가 발화): pendingText 누적', () => {
  const s = { phase: 'confirming' as const, micOn: true, pendingText: '안녕', confirmGate: true, signalGating: false };
...
it('FINAL_RESULT(게이트 ON, speaking 중): 무시', () => {
  const s = { phase: 'speaking' as const, micOn: true, pendingText: '', confirmGate: true, signalGating: false };
  const r = handsFreeReducer(s, { type: 'FINAL_RESULT', text: '끼어들기' });
  expect(r.state.phase).toBe('speaking');
  expect(r.effects).not.toContain('SAY');
});
...
it('CONFIRM_SEND(pendingText 비어있음): listening 복귀, SAY 없음', () => {
  const s = { phase: 'confirming' as const, micOn: true, pendingText: '   ', confirmGate: true, signalGating: false };
  const r = handsFreeReducer(s, { type: 'CONFIRM_SEND' });
  expect(r.state.phase).toBe('listening');
  expect(r.effects).not.toContain('SAY');
});
...
it('CANCEL_SEND(confirming): listening + pendingText 폐기, effect 없음', () => {
  ...
  expect(r.state).toEqual({ phase: 'listening', micOn: true, pendingText: '', confirmGate: true, signalGating: false });
  expect(r.effects).toEqual([]);
});

it('CLONE_SPEAKING(sending): speaking', () => {
  const s = { phase: 'sending' as const, micOn: true, pendingText: '', confirmGate: false, signalGating: false };
...
it('RESPONSE_END(speaking, micOff): paused, START_STT 없음', () => {
  const s = { phase: 'speaking' as const, micOn: false, pendingText: '', confirmGate: false, signalGating: false };
  const r = handsFreeReducer(s, { type: 'RESPONSE_END' });
  expect(r.state.phase).toBe('paused');
  expect(r.effects).not.toContain('START_STT');
});
...
it('MIC_OFF(어느 상태든): paused + STOP_STT + STOP_DETECTOR', () => {
  ...
  expect(r.state).toEqual({ phase: 'paused', micOn: false, pendingText: '', confirmGate: false, signalGating: false });
  expect(r.effects).toEqual(expect.arrayContaining(['STOP_STT', 'STOP_DETECTOR']));
});

it('MIC_ON(paused): listening + START_STT', () => {
  const s = { phase: 'paused' as const, micOn: false, pendingText: '', confirmGate: false, signalGating: false };
  const r = handsFreeReducer(s, { type: 'MIC_ON' });
  expect(r.state).toEqual({ phase: 'listening', micOn: true, pendingText: '', confirmGate: false, signalGating: false });
  expect(r.effects).toContain('START_STT');
});
...
it('CALL_ENDED: micOn 리셋(paused→idle에서 다음 통화 마이크 ON 기본)', () => {
  const s = { phase: 'paused' as const, micOn: false, pendingText: '', confirmGate: true, signalGating: false };
  const r = handsFreeReducer(s, { type: 'CALL_ENDED' });
  expect(r.state).toEqual({ phase: 'idle', micOn: true, pendingText: '', confirmGate: false, signalGating: false });
});

it('MIC_OFF(confirming): paused + pendingText 폐기', () => {
  const s = { phase: 'confirming' as const, micOn: true, pendingText: '보내려던 말', confirmGate: true, signalGating: false };
  const r = handsFreeReducer(s, { type: 'MIC_OFF' });
  expect(r.state).toEqual({ phase: 'paused', micOn: false, pendingText: '', confirmGate: true, signalGating: false });
  expect(r.effects).toEqual(expect.arrayContaining(['STOP_STT', 'STOP_DETECTOR']));
});
...
describe('greeting phase', () => {
  ...
  it('greeting + SPEECH_START → speaking', () => {
    const g = { phase: 'greeting' as const, micOn: true, pendingText: '', confirmGate: false, signalGating: false };
  ...
  it('SPEECH_START는 greeting 외엔 무시(중복 인사 방지)', () => {
    const sp = { phase: 'speaking' as const, micOn: true, pendingText: '', confirmGate: false, signalGating: false };
  ...
  it('greeting + GREET_TIMEOUT, micOff면 paused로 빠지지 않고 greeting 유지', () => {
    const g = { phase: 'greeting' as const, micOn: false, pendingText: '', confirmGate: false, signalGating: false };
    const r = handsFreeReducer(g, { type: 'GREET_TIMEOUT' });
    expect(r.state.phase).toBe('greeting');
  });
});
