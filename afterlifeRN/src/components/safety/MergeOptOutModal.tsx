...
export default function MergeOptOutModal({
  ...
  const deadlineStr = new Date(deadlineAt).toLocaleString("ko-KR");
  return (
    <Modal visible={visible} animationType="fade" transparent onRequestClose={onDismiss}>
      <Pressable style={s.backdrop} onPress={onDismiss}>
        <Pressable style={s.card} onPress={() => {}}>
          <Text style={s.title}>
            {t("merge.title", { defaultValue: "클론 병합 확인 필요" })}
          </Text>
          <Text style={s.body}>
            {t("merge.body", {
              source: sourceCloneName,
              target: targetCloneName,
              deadline: deadlineStr,
              defaultValue:
                "{{source}}이(가) {{target}}와 병합될 예정입니다.\n\n병합 결과 원본 클론은 아카이브(archived_merged)되고 새로운 통합 클론이 생성됩니다. 공유자 전원의 응답이 필요하며, 기한({{deadline}})까지 응답이 없으면 기본값(비동의)으로 처리됩니다.\n\n원본 클론은 복구 가능한 상태로 보존되므로, 추후 이의를 제기할 수 있습니다.",
            })}
          </Text>

          <View style={s.btnRow}>
            <Button
              title={t("merge.agree", { defaultValue: "동의" })}
              onPress={() => onDecision("agree")}
            />
            <View style={{ height: 10 }} />
            <Button
              title={t("merge.decline", { defaultValue: "거절" })}
              variant="ghost"
